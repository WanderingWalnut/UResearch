...
flowchart LR
  web["<b>Web App</b><br/>Next.js client<br/>Discovery, campaigns, templates, Outreach Board"]

  subgraph app["App Server"]
    direction TB
    identity["<b>IdentityModule</b><br/>TypeScript<br/>Google sign-in, Student profile"]
    ...
    board["<b>OutreachBoardModule</b><br/>TypeScript<br/>Board cards, status chips, follow-up reminders"]
    delivery["<b>DeliveryModule</b><br/>TypeScript<br/>Gmail send-only"]
  end
  ...
  board -->|"thread rows"| db
  board -->|"campaign context"| campaigns
  ...
  class identity,discovery,campaigns,board,delivery appNode
...
flowchart LR
  ...
  subgraph workers["Workers"]
    ...
    send_worker["<b>CampaignSendWorker</b><br/>Edge Function<br/>Gmail send, update message + thread"]
    ingest_worker["<b>ProfileIngestWorker</b><br/>Edge Function<br/>Crawl profiles, embeddings, FTS index"]
    reply_worker["<b>ReplyStateWorker</b><br/>Deferred post-launch<br/>Only if mailbox reading is adopted"]
  end
  ...
  queue -->|"future reply jobs"| reply_worker
  ...
  reply_worker -->|"future reply strategy"| mail_provider
...
src/
  app/                          # Next.js routes (thin)
    (auth)/
    api/webhooks/mail/          # Future reply-state webhook if mailbox-read strategy supports it
    api/tracking/[token]/       # Open pixel (slice 5)
  modules/
    identity/                   # Slice 1
    discovery/                  # Slice 2
    campaigns/                  # Slice 3
    delivery/                   # Slice 3 Gmail send-only
    outreach-board/             # Slice 4
  workers/                      # Supabase Edge Function entrypoints
    campaign-send/
    profile-ingest/
    reply-state/                # Future, only if reply strategy needs workers
    campaign-dispatch/
supabase/
...
| **OutreachBoardModule** | `outreach_threads`, `thread_messages`, `follow_up_reminders` | CampaignModule (read context) | Delivery provider |
| **DeliveryModule** | `mailbox_connections` | Gmail API | Domain campaign/thread mutations outside send result handling |
...
| --- | --- |
| 1 Student foundation | `identity/`, `app/(auth)/` |
| 2 Discovery | `discovery/`, `workers/profile-ingest/` |
| 3 Campaign send | `campaigns/`, `workers/campaign-send/`, `workers/campaign-dispatch/` |
| 4 Outreach Board + reply tracking | `outreach-board/`, `delivery/`, `workers/reply-state/`, `app/api/webhooks/mail/` if future mailbox reading is added |
...
| Reply-state webhook | App Server route | Added only if future mailbox-reading strategy supports events |
